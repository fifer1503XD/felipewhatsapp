...
export const login = (provider, email, password,userMessenger) => {
    return (dispatch) => {
        return new Promise(async(resolve, reject) => {
            try{
                if(provider === "email"){
                    ...
                    dispatch(setUser(user)); 
                    dispatch(verifyRegisterApi(user,userMessenger));
                    console.log(user)                   
                }
                resolve(true);
            }catch(error){
...
export const postNewUsser =async (email,name, lastname,uid) => {
    ...
    export const  verifyRegisterApi=(user,userMessenger)=>{
        let exist= userMessenger.filter((userMessenger)=>
            user.uid===userMessenger.uid )
            if (exist===''){
                alert('hay que registrar'+exist)
                postNewUsser(user.email,user.displayName,'.',user.uid)
            }
            else{
                alert('no hay nada'+exist)
            }
    }
